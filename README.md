...
This project, NeuroHex, is an autonomous robot that leverages an Artificial Neural Network (ANN) for intelligent navigation and environment perception. Designed to handle complex and uneven terrains, the robot uses a fusion of hardware and software components to perceive its surroundings, detect objects, and navigate autonomously. You can also conveniently see the [PDF of Project Report](Latex_Report.pdf) for more info.
...
There is nothing as Boot loader as the Pi's ROM is directly loaded an OS. 
Connect the Pi board to the peripherals such as *Desktop , Mouse and Keyboard* Now you can confiure the raspberry pi. 
Give a custom **Username** and **Password**. Configure WLAN (WIFI) with a known network so that it connects automatically when booted ON. 
This is very essential as this the way we will be going to work in further stages by enabling SSH. 
In services enable **SSH** and **VNC**. 
Once it gets connected to the wifi you can unplug the peripherals and can use remotely using any Third Party Softwares -> CLI([Putty](https://putty.software/), Remote Resktop Connection by Microsoft, bash/Powershell/Command Prompt) or GUI Desktop ([RealVNC](https://www.realvnc.com/en/), VNC Server or anyother softwares). 
